<script src="<c:url value='/resources/js/jquery-1.11.1.min.js'></c:url>"></script>
        <script src="<c:url value='/resources/js/bootstrap.js'></c:url>"></script>

        <script src="<c:url value='/resources/js/bootstrap-datepicker.js'></c:url>"></script>
        <script src="<c:url value='/resoruces/js/bootstrap-table.js'></c:url>"></script>
        
         <script src="<c:url value='/resoruces/js/angular.min.js'></c:url>"></script>
        <script src="<c:url value='/resoruces/js/controller/registrationController.js'></c:url>"></script>
        
        
        <script src="<c:url value='/resources/js/script.js'></c:url>"></script>	
</body>

</html>